...
class Smartwave_Legenda_Model_Import_Demoversion extends Mage_Core_Model_Abstract
{
	private $_importPath;
    ...
    {
        parent::__construct();
		$this->_importPath = Mage::getBaseDir() . '/app/code/local/Smartwave/Legenda/etc/import/';
    }
	...
	public function importDemoversion($demoversion,$store=NULL,$website = NULL)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('legenda')->__("Can't get the data file for import demoversion: %s", $xmlPath)
                );
			}
