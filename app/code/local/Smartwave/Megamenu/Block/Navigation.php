...
class Smartwave_Megamenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    public function drawMegaMenuItem($category,$mode = 'dt', $level = 0, $last = false)
    {
        ...
        if ($drawPopup) {
            ...
            $html[] = '<a itemprop="url" href="'.$this->getCategoryUrl($category).'" class="'.$active.'"><span itemprop="name">'.$name.$catLabel.'</name></a>';
            if ($mode != 'mb') {
                $html[] = '<div class="nav-sublist-dropdown" style="display: none;">';
                $html[] = '<div class="container">';               
            }
            if ($level == 0 && $blockType == 'wide') {
                if ($block_top)
                ...
                if (count($activeChildren)) {
                    ...
                    if (!$columns || ($columns == 0))
                        //$columns = 6; //updated from version 1.1.0
            ...
            $html[] = '<li>';
            $html[] = '<a itemprop="url" href="'.$this->getCategoryUrl($category).'" class="'.$active.'"><span itemprop="name">'.$name.'</span></a>';                
            $html[] = '</li>';
        }
        $html = implode("\n", $html);
        return $html;
