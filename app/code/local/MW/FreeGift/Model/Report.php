<?php
class MW_FreeGift_Model_Report extends Mage_Core_Model_Abstract
{
    ...
    public function prepareCollection($data)
    {
        ...
        if($data['report_range'] == MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_CUSTOM)
        {
        ...
        $collection->addFieldToFilter('main_table.status', MW_FreeGift_Model_Statusreport::COMPLETE);
...
//            array_push($temp,$co_order->getTotalOrderSum());
        ...
        $collection_temp_order = Mage::getModel('sales/order')->getCollection();
        $collection_temp_order->removeAllFieldsFromSelect();
        $collection_temp_order->addFieldToFilter('status', MW_FreeGift_Model_Statusreport::COMPLETE);
        ...
        foreach($collection_temp_order as $order){
            ...
            foreach($order_id_array as $order_id){
                ...
                $collection_order_temp = Mage::getModel('sales/order')->load($order_id);
                $customer_id =  $collection_order_temp->getCustomerId();
                array_push($number_customer_array,$customer_id);
	
                $collection_order_item = Mage::getModel('sales/order_item')
                    ->getCollection()
                ...
                foreach($collection_order_item as $product_gift){
                    ...
                    $pro=Mage::getModel('catalog/product')->load($product_gift_id);
        ...
        {
            case MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_24H:
                ...
                $rangeDate = $this->_buildArrayDate(MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_24H, $start_time['h'], $start_time['h'] + 24, $start_time);
                ...
                break;
            case MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_WEEK:
                $start_time = strtotime("-6 day", strtotime("Sunday Last Week"));
                $startDay = date('d', $start_time);
                $endDay = date('d',strtotime("Sunday Last Week"));
                $rangeDate = $this->_buildArrayDate(MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_WEEK, $startDay, $endDay);
                ...
                break;
            case MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_MONTH:
                ...
                $rangeDate = $this->_buildArrayDate(MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_MONTH, $start_day, $end_day);
                ...
                break;
            case MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_7DAYS:
            case MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_30DAYS:
                if($data['report_range'] == MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_7DAYS)
                {
                    $last_x_day = 7;
                }
                else if($data['report_range'] == MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_LAST_30DAYS)
                {
                    $last_x_day = 30;
                }
                ...
                $rangeDate = $this->_buildArrayDate(MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_CUSTOM, 0, 0, $original_time);

                $_data = $this->_buildResult($collection_gift, $collection_order, $collection_order, 'multiday', $rangeDate, $original_time);
                break;
            case MW_FreeGift_Model_Adminhtml_Const::REPORT_RAGE_CUSTOM:
                $original_time = array(
                    'from'  => $data['from'],
                    'to'    => $data['to']
                );
        ...
        $_data['title'] = Mage::helper('freegift')->__('FreeGift / Report FreeGift');
        ...
        $_data['statistics']['total_order'] = Mage::helper('core')->currency($total_order, true, false);
        $_data['statistics']['total_gift'] = Mage::helper('core')->currency($number_product_gift, true, false);
        $_data['statistics']['number_customer'] = Mage::helper('core')->currency($number_customer, true, false);
        $_data['statistics']['avg_gift_per_customer'] = Mage::helper('core')->currency(round($number_product_gift/$number_customer ,2), true, false);
        $_data['statistics']['avg_gift_per_order'] = Mage::helper('core')->currency(round($number_product_gift/$total_order_number ,2), true, false);
                    ...
                    foreach($collection_gift as $redeemd)
                    {
                        if((int)$redeemd[$type] == $count)
                        {
                            if(isset($date['day']) && $date['day'] == (int)$redeemd['day'])
                            {
                                ...
                                $_data['report']['redeemed'][$i] = Mage::helper('core')->currency((int)$redeemd['total_gift_sum'], false, false);
                    ...
                    foreach($collection_order as $reward)
                    {
                        if((int)$reward->{"get$type"}() == $count)
                        {
                            if(isset($date['day']) && $date['day'] == (int)$reward->getDay())
                            {
                                ...
                                $_data['report']['rewarded'][$i] = Mage::helper('core')->currency((int)$reward->getTotalOrderSum(), false, false);
                            }
                            else if(!isset($date['day']))
                            {
                                //$_data['report']['rewarded'][$i] = (int)$reward->getTotalOrderSum() ;
